    ...
    return (
        <article>
            <header style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <h1 style={{ fontSize: '28px', fontWeight: 700 }}>favorite people</h1>
            </header>

            <ul style={{ display: 'flex', flexDirection: 'column', gap: '24px', fontSize: '15px' }}>
                {people.map((person, i) => (
                    <li key={i}>
                        <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '4px' }}>{person.name}</h3>
                        <p style={{ color: 'var(--text-secondary)' }}>{person.description}</p>
                    </li>
                ))}
            </ul>
        </article>
